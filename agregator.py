...
class Agregator:
    def __init__(self) -> None:
        ...
        self.register_agent = mqtt.Client(
            client_id=str(self.uuid), transport="websockets"
        )
        ...
        self.register_agent.connect(
            self._config["mqtt"]["broker"], int(self._config["mqtt"]["broker_port"])
        )
    ...
    def selection(self, selection: str, group_function: str) -> pd.DataFrame:
        ...
        if group_function != "":
            temp_memory = eval(
                "temp_memory."
                + str(group_function)
                + "(axis=0, numeric_only=True).to_frame().T"
            )
        return temp_memory
    ...
    def package(self):
        pack = Queue()
        if (
            self._config["constraints"]["select"] != ""
            or self._config["constraints"]["function"] != ""
        ):
            data = self.selection(
                self._config["constraints"]["select"],
                self._config["constraints"]["function"],
            )
        else:
    ...
    def register(self):
        self.register_agent.publish(
            "agreg_register_8678855",
            json.dumps(
                {
                    "uuid": str(self.uuid),
                    "config": self._config,
    ...
    def http(self):
        ...
        while self.sending:
            if data.qsize() != 0:
                ...
                content = (
                    "http://"
                    + self._config["http"]["destiantion"]
                    + ":"
                    + str(self._config["http"]["destiantion_port"])
                    + str(self._config["http"]["destiantion_path"])
                )
    ...
    def mqtt(self):
        ...
        while self.sending:
            if data.qsize() != 0:
                content = (
                    self._config["mqtt"]["send_topic"],
                    json.dumps({"data": data.get()}),
                )
